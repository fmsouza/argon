...
pub(crate) fn js_polyfill(module: &str, name: &str) -> Option<&'static str> {
    match (module, name) {
        ...
        // std:ws polyfills — wrap WebSocket API (Node.js ws package or browser WebSocket)
        ("ws", "wsConnect") => Some(
            "function(url) { try { const WS = typeof WebSocket !== 'undefined' ? WebSocket : require('ws'); const ws = new WS(url); return new Promise((resolve) => { ws.onopen = () => { resolve({ value: { __ws: ws, send(data) { try { ws.send(data); return { value: undefined, isOk: true, isErr: false }; } catch(e) { return { error: { code: 'WS_ERROR', message: e.message }, isOk: false, isErr: true }; } }, sendBytes(data) { try { ws.send(Buffer.from(data)); return { value: undefined, isOk: true, isErr: false }; } catch(e) { return { error: { code: 'WS_ERROR', message: e.message }, isOk: false, isErr: true }; } }, recv() { return new Promise((res) => { ws.once('message', (msg) => { const isText = typeof msg === 'string'; res({ value: { data: isText ? msg : '', bytes: isText ? [] : Array.from(msg), isText, isBinary: !isText }, isOk: true, isErr: false }); }); }); }, ping() { try { if (ws.ping) ws.ping(); return { value: undefined, isOk: true, isErr: false }; } catch(e) { return { error: { code: 'WS_ERROR', message: e.message }, isOk: false, isErr: true }; } }, close(code, reason) { ws.close(code || 1000, reason || ''); return { value: undefined, isOk: true, isErr: false }; }, isOpen() { return ws.readyState === 1; } }, isOk: true, isErr: false }); }; ws.onerror = (e) => { resolve({ error: { code: 'WS_ERROR', message: e.message || 'connection failed' }, isOk: false, isErr: true }); }; }); } catch(e) { return { error: { code: 'WS_ERROR', message: e.message }, isOk: false, isErr: true }; } }",
        ),
        ("ws", "wsListen") => Some(
            "function(addr, port) { try { const WSServer = require('ws').Server; const wss = new WSServer({ host: addr, port }); return { value: { __wss: wss, accept() { return new Promise((resolve) => { wss.once('connection', (ws) => { resolve({ value: { __ws: ws, send(data) { try { ws.send(data); return { value: undefined, isOk: true, isErr: false }; } catch(e) { return { error: { code: 'WS_ERROR', message: e.message }, isOk: false, isErr: true }; } }, recv() { return new Promise((res) => { ws.once('message', (msg) => { const isText = typeof msg === 'string'; res({ value: { data: isText ? msg : msg.toString('utf8'), bytes: [], isText: true, isBinary: false }, isOk: true, isErr: false }); }); }); }, close(code, reason) { ws.close(code || 1000, reason || ''); return { value: undefined, isOk: true, isErr: false }; }, isOpen() { return ws.readyState === 1; } }, isOk: true, isErr: false }); }); }); }, close() { wss.close(); return { value: undefined, isOk: true, isErr: false }; }, addr() { const a = wss.address(); return a ? a.address + ':' + a.port : ''; } }, isOk: true, isErr: false }; } catch(e) { return { error: { code: 'WS_ERROR', message: e.message }, isOk: false, isErr: true }; } }",
        ),
