...
impl JsCodegen {
    ...
    fn generate_call(&mut self, c: &CallExpr) -> Result<(), CodegenError> {
        ...
        if let Expr::Identifier(id) = c.callee.as_ref() {
            match id.sym.as_str() {
                "Some" => {
                    self.output.push_str("new SafeScript.Option.Some(");
                ...
                "None" => {
                    self.output.push_str("new SafeScript.Option.None()");
                    return Ok(());
                }
                "Ok" => {
                    self.output.push_str("new SafeScript.Result.Ok(");
                    for (i, arg) in c.arguments.iter().enumerate() {
                        if i > 0 {
                            self.output.push_str(", ");
                        }
                ...
                "Err" => {
                    self.output.push_str("new SafeScript.Result.Err(");
                    for (i, arg) in c.arguments.iter().enumerate() {
                        if i > 0 {
                            self.output.push_str(", ");
                        }
                ...
                "Vec" => {
                    self.output.push_str("new SafeScript.Vec(");
                    for (i, arg) in c.arguments.iter().enumerate() {
                        if i > 0 {
                            self.output.push_str(", ");
                        }
                ...
                "Shared" => {
                    self.output.push_str("SafeScript.Shared.wrap(");
                    for (i, arg) in c.arguments.iter().enumerate() {
                        if i > 0 {
                            self.output.push_str(", ");
                        }
        ...
        // Check if this is a SafeScript.Option.Some, SafeScript.Result.Ok, etc. call
...
pub fn generate_type_declarations(source: &SourceFile) -> String {
    ...
    output.push_str("// TypeScript declarations generated by SafeScript\n\n");
...
fn type_to_ts_string(ty: &safescript_ast::Type) -> String {
    match ty {
        ...
        safescript_ast::Type::Ref(r) => type_to_ts_string(&r.ty),
        safescript_ast::Type::MutRef(r) => type_to_ts_string(&r.ty),
