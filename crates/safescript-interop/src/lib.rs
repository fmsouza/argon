...
//! Handles safe interoperability between SafeScript and JavaScript
...
impl InteropGenerator {
    ...
    pub fn generate_wrapper(&self, import: &JsImport) -> String {
        ...
        format!(
            r#"function {}(...args) {{
    return SafeScript.Shared.wrap({}, ...args);
}}"#,
    ...
    pub fn generate_import_wrapper(&self, import: &JsImport) -> String {
        ...
        format!("SafeScript.Shared.wrap({})", import.local_name)
    }

    pub fn generate_export_wrapper(&self, export: &JsExport) -> String {
        format!("SafeScript.export({})", export.name)
    }

    pub fn map_safescript_to_js(&self, ty: &Type) -> String {
        match ty {
    ...
    pub fn map_js_to_safescript(&self, js_type: &str) -> String {
        match js_type {
    ...
    pub fn generate_declaration_file(&self) -> String {
        ...
        for export in &self.exports {
            output.push_str(&format!(
                "export function {}: {};\n",
                export.name,
                self.map_safescript_to_js(&export.ty)
            ));
        }
